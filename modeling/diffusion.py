...
class DiffusionModel(nn.Module):
    def __init__(
        ...
        for name, schedule in get_schedules(betas[0], betas[1], num_timesteps, device).items():
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        timestep = torch.randint(1, self.num_timesteps + 1, (x.shape[0],), device=self.device)
    ...
    def sample(self, num_samples: int, size, device) -> torch.Tensor:
        ...
        for i in range(self.num_timesteps, 0, -1):
            z = torch.randn(num_samples, *size, device=self.device) if i > 1 else 0
...
def get_schedules(beta1: float, beta2: float, num_timesteps: int, device: str) -> Dict[str, torch.Tensor]:
    assert beta1 < beta2 < 1.0, "beta1 and beta2 must be in (0, 1)"

    betas = (beta2 - beta1) * torch.arange(0, num_timesteps + 1, dtype=torch.float32, device=device) / num_timesteps + beta1
    ...
    sqrt_one_minus_alpha_prod = torch.sqrt(1 - alphas_cumprod).to(device)
